...
router.get("/posts/:_postId/comments", async (req, res) => {
  ...
  try {
    ...
    const comments = await Comments.find({ postId }).select(
      "commentId nickname content createdAt -_id"
    );
    // 작성 날짜 기준으로 내림차순 정렬하기
...
// 2. 댓글 작성
router.post("/posts/:_postId/comments", auth, async (req, res) => {
  ...
  let lastCommentId = await Comments.find({})
    ...
    .limit(1)
    .select("commentId");
  const commentId = lastCommentId.length
    ? lastCommentId[0]["commentId"] + 1
    : 1;
...
// 3. 댓글 수정
router.put("/posts/:_postId/comments/:_commentId", auth, async (req, res) => {
  // 미들웨어에서 사용자 확인된 정보를 받아서,
  const { nickname } = res.locals.nickname;

...
router.get("/posts", async (req, res) => {
    // 전체 게시글 중에 postId, nickname, title, createdAt 만 추출
    const posts = await Posts.find({})
        .select("postId nickname title createdAt -_id");
    // 작성 날짜 기준으로 내림차순 정렬하기
...
router.post("/posts", auth, async (req, res) => {
    ...
    // DB에 추가
    await Posts.create({title, content, postId, createdAt, nickname })
    // 201, 게시글 생성 메시지 반환
...
router.put("/posts/:_postId", auth, async (req, res) => {
    // 미들웨어에서 사용자 확인된 정보를 받아서,
    const { nickname } = res.locals.nickname
    ...
    try {
        ...
        if (result.length) {
            if (nickname === result[0].nickname) {
                // body내용으로 DB 업데이트하고,
                await Posts.updateOne({ postId: _postId }, { $set: { title, content } })
                // 201, 수정 완료 메시지 반환
...
router.delete("/posts/:_postId", auth, async (req, res) => {
    // 미들웨어에서 사용자 확인된 정보를 받아서,
    const { nickname } = res.locals.nickname
    // 파라미터로 들어온 _postId를 필드 타입인 Number로 변환해서 저장하고,
    const _postId = Number(req.params._postId)
